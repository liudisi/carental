...
<script>
    ...
    layui.extend({
        ...
        tableIns = table.render({
            elem: '#roleTable'
            , url: '${requestScope.contextPath}/role/findAllRole'
            , toolbar: '#roleToolBar'
            , title: '角色列表'
            , height: 'full-140'
        ...
        $("#doSearch").on('click', function () {
            ...
            tableIns.reload({
                url: "${requestScope.contextPath}/role/findAllRole?" + params,
                page:{curr:1}
            })
        });
        ...
        function openAdd() {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    //清空表单数据
                    $("#dataFrm")[0].reset();
                    url = "${requestScope.contextPath}/role/addRole";
                }
            });
        }
        ...
        function batchDelete() {
            ...
            layer.confirm('确定删除选中的这些角色吗？', function (index) {
                //向服务端发送删除指令
                $.post("${requestScope.contextPath}/role/batchDelete", params, function (res) {
                    layer.msg(res.msg);
                    //刷新数据 表格
                    tableIns.reload();
                });
        ...
        table.on('tool(roleTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('确定删除【' + data.roleName + '】这个角色吗？', function (index) {
                    //向服务端发送删除指令
                    $.get("${requestScope.contextPath}/role/deleteRoleByRid?rid=" + data.roleId, function (res) {
                        layer.msg(res.msg);
                        //刷新数据 表格
                        tableIns.reload();
                    });
        ...
        function openUpdateRole(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    url = "${requestScope.contextPath}/role/updateRole";
                }
            });
        }

        //打开分配菜单页面
        function selectRoleMenu(data) {
            var menuTree;
            mainIndex = layer.open({
                type: 1,
                ...
                yes: function (index, layero) {
                    ...
                    var params = "rid=" + roleId;
                    $.each(nodes, function (index, item) {
                        params += "&mid=" + item.nodeId;
                    });
                    $.post("${requestScope.contextPath}/role/addMenuRole", params, function (obj) {
                        layer.msg(obj.msg);
                        //关闭弹出层
                ...
                success: function (index) {
                    ...
                    menuTree = dtree.render({
                        ...
                        checkbarType: "all",
                        checkbarData: "choose",
                        url: "${requestScope.contextPath}/role/initRoleMenuTreeJson?roleId=" + data.roleId // 使用url加载（可与data加载同时存在）
                    });
                }
            });
        }
